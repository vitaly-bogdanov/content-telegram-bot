...
import { setCacheMessageIdsHelper, initializeCacheStateHelper, getCacheMessageIdsHelper } from '../../lib/cache/index.js'
...
export const adminCommand = async function(ctx) {
  ...
  if (cacheMessageIds.length) deleteMessagesHelper(this, id, cacheMessageIds);
  ...
  this.once('message', async (ctx) => {
    ...
    if (loginSuccess) { // успешная авторизация
      deleteMessagesHelper(this, id, getCacheMessageIdsHelper(id));
